...
| `ROUTER_TLS_HOSTNAME` | required | Primary hostname the router serves TLS for |
| `ACME_EMAIL` | required | Contact email registered with Let's Encrypt |
| `ACME_STAGING` | `false` | `true` to use Let's Encrypt staging for app hostnames |
| `WIREGUARD_PORT` | `51823` | WireGuard listen port for the mesh |
| `ROUTER_THREADS` | — | Number of Pingora worker threads |
| `RPS_LIMIT` | — | Optional requests-per-second rate limit |
| `API_UPSTREAM_TARGETS` | — | Override targets for `api.mikrom.spluca.org` proxy |
| `WEB_UPSTREAM_TARGETS` | — | Override targets for the dashboard proxy |
