---
title: "Managing PostgreSQL Databases with Neon"
...
<Note>
  The `--memory` and `--disk` flags both accept **integer MiB values**. `512M`, `1G`, `2G`, and `4G` are the valid memory presets (the CLI converts them to MiB internally). The `--disk` flag takes a raw MiB integer — for example, `--disk 2048` provisions a 2 GiB disk.
</Note>
...
To retrieve the same information as structured data for scripting:

```bash
mikrom db connection <id> --output json
```

```json
{
  "ssh_tunnel": "ssh -L 5432:localhost:5432 mikrom@<vm-ip> -i ~/.ssh/mikrom_key -N",
  "psql": "psql \"host=localhost port=5432 dbname=orders user=mikrom\"",
  "host": "localhost",
  "port": 5432,
  "dbname": "orders",
  "user": "mikrom"
